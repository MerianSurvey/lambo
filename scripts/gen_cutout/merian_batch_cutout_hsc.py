...
https://github.com/MerianSurvey/caterpillar/blob/main/caterpillar/lsstpipe/cutout.py
"""
...
def _get_output_dir(output_dir, chunk_arr, name_arr):
    ...
    if not os.path.isdir(output_dir):
        raise ValueError(
            "Output directory '{:s}' does not exist".format(output_dir))
...
def _prepare_input_cat(input_cat, half_size, unit, ra_col, dec_col, id_col, chunk,
    ...
    if isinstance(half_size, str):
        if half_size.strip() not in input_cat.colnames:
            raise ValueError(
                "Wrong half size column name. [{:s}]".format(half_size))
    ...
    if id_col is None:
        name_arr = _get_ra_dec_name(
            np.arange(len(ra_arr)) + 1, ra_arr, dec_arr)
    else:
    ...
    if chunk is not None:
        if isinstance(chunk, str):
            if chunk not in input_cat.colnames:
                raise ValueError(
                    "Wrong Chunk column name. [{:s}]".format(chunk))
    ...
    sample = QTable(
        [name_arr, prefix_arr, dir_arr, chunk_arr, list(
            ra_arr), list(dec_arr), half_size_arr],
...
def prepare_sample(input_cat, root, collections, skymap_name, ready=False, save=True, half_size=10, unit='arcsec',
    ...
    skymap_name : str
        The name of the skymap. Default is 'hsc_rings_v1'. If you use DECam native pixel size, set it to 'decam_rings_v1'.
    band : str
        Name of the filter.
    ...
    # Get the butler for the given dataset and return the skyMap object.
    butler, skymap = _prepare_dataset(root, collections, skymap_name)
...
def cutout_one(butler, skymap, skymap_name, obj, band, data_type, psf, verbose):
    ...
    img_fits = os.path.join(
        dir, "{:s}_{:s}_{:s}.fits".format(prefix, band, data_type))
    psf_fits = os.path.join(dir, "{:s}_{:s}_psf.fits".format(prefix, band))

    cutout = generate_cutout(
        butler, skymap, skymap_name, ra, dec, band=band, data_type=data_type,
        half_size=half_size, psf=psf)

    if cutout is None:
        if verbose:
            print("# No data for {:s} at {:.5f} {:.5f}".format(
                str(obj['name']), ra, dec))
        return 0
...
def cutout_batch(input_cat, root, collections, skymap_name, band, njobs=1, psf=True, ready=False, save=True, half_size=10,
    ...
    butler, skymap, sample = prepare_sample(
        input_cat, root, collections, skymap_name, ready=ready, save=save, half_size=half_size, unit=unit,
    ...
    if data_type.strip() not in ['deepCoadd', 'deepCoadd_calexp', 'deepCoadd_background', 'deepCoadd_calexp_background']:
        raise ValueError(
            "Wrong coadd type. [deepCoadd, deepCoadd_calexp, deepCoadd_background, deepCoadd_calexp_background]")

    if njobs <= 1:
        flags = [cutout_one(butler, skymap, skymap_name, obj, band,
                            data_type, psf, verbose) for obj in sample]
        sample['flag'] = flags
        return sample
    else:
        Parallel(n_jobs=njobs)(delayed(cutout_one)(butler, skymap, skymap_name,
                                                   obj, band, data_type, psf, verbose) for obj in sample)
    return sample


def make_cutout():
    """
    Make cutout for a given catalog.
    """
    root = '/projects/MERIAN/repo'
    skymap_name = 'hsc_rings_v1'
    data_type = 'deepCoadd_calexp'

    hsc_cosmos = "HSC/runs/RC2/w_2022_04/DM-33402"
    merian_collection = 'DECam/runs/merian/w_2022_02/t9813_deep'

    # input_cat = os.path.join(output, '/Cutout/stars/stars_obj_cat_joint.fits')
    output = '/scratch/gpfs/jiaxuanl/Data/Merian/Cutout/cosmos_gal/'

    input_cat = '/scratch/gpfs/jiaxuanl/Data/Merian/Catalogs/t9813_deep/COSMOS_lowz_hsc_merian.fits'

    today = date.today()

    for filt in list('grizy') + ['N708', "N540"]:
        if filt in ['N708', "N540"]:
            collection = merian_collection
        else:
            collection = hsc_cosmos
        sample_hsc = cutout_batch(
            input_cat, root, collection, skymap_name, filt, njobs=2, psf=True, ready=False, save=True, half_size='half_size',
            unit='arcsec', ra_col='coord_ra', dec_col='coord_dec', id_col='ID_yifei',
            data_type=data_type, output_dir=output, prefix='cosmos', chunk=90, verbose=True)

        sample_hsc.write(os.path.join(
            output, f'COSMOS-{today.year:4d}-{today.month:02d}-{today.day:02d}_{filt}_done.fits'), overwrite=True)
