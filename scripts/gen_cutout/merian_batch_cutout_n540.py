...
PIXEL_SCALE = 0.262  # 0.168  # arcsec / pixel
...
def _prepare_dataset(root, collections):
    ...
    butler = dafButler.Butler(
        root, collections=collections, skymap="decam_rings_v1")
...
def _get_patches(butler, skymap, coord_list, band, data_type='deepCoadd'):
    ...
    images = []
    for t, p in patches:
        data_id = {'tract': t, 'patch': p, 'band': band}
        try:
...
def generate_cutout(butler, skymap, ra, dec, band='N708', data_type='deepCoadd',
    ...
    img_patches = _get_patches(
        butler, skymap, radec_list, band, data_type=data_type)
...
def cutout_batch(input_cat, root, collections, band, njobs=1, psf=True, ready=False, save=True, half_size=10,
    ...
    if njobs <= 1:
        flags = [cutout_one(butler, skymap, obj, band,
                            data_type, psf, verbose) for obj in sample]
        sample['flag'] = flags
        return sample
    else:
        Parallel(n_jobs=njobs)(delayed(cutout_one)(butler, skymap,
...
def test_cutout():
    ...
    n708 = 'DECam/runs/merian/w_2022_02/t9813_deep_native'
    n540 = 'DECam/runs/merian/w_2022_02/t9813_deep_native'

    input_cat = os.path.join(output, 'stars_obj_cat_joint_native.fits')
    ...
    sample_n708.write(os.path.join(
        output, f'stars-{today.year:4d}-{today.month:02d}-{today.day:02d}_n708_native_done.fits'), overwrite=True)
    ...
    sample_n540.write(os.path.join(
        output, f'stars-{today.year:4d}-{today.month:02d}-{today.day:02d}_n540_native_done.fits'), overwrite=True)
