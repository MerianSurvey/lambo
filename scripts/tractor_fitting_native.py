...
def fitting_obj(index, obj_cat, meas_cat, point_source=False,
    ...
    try:
        ...
        hsc_data = Data(images=images, weights=weights, wcs=w,
                        psfs=psf_pad, channels=hsc_channels)
        ...
        merian_data = Data(images=images, weights=weights, wcs=w,
                           psfs=psf_pad, channels=merian_channels)
    ...
    try:
        ...
        model_dict[ref_filt], _obj_cat_i = tractor_hsc_sep_blob_by_blob(
            obj, ref_filt, hsc_data.channels, hsc_data, point_source=point_source,
        ...
        # Do forced photometry for all bands
        for filt in hsc_channels:
            pos = True
            fix_all = True
            print('filt = ', filt)
        ...
        x, y = merian_data.wcs.wcs_world2pix(
            *hsc_data.wcs.wcs_pix2world(ref_source.pos.x, ref_source.pos.y, 0), 0)
        ...
        print(
            f'FITTING ERROR FOR {index} = obj {obj[ID_name]}', e, traceback.print_exc())
    try:
        row = _write_to_row(row, model_dict, channels=channels)
    except Exception as e:
        print(
            f'MEAUREMENT ERROR FOR {index} = obj {obj[ID_name]}', e, traceback.print_exc())
